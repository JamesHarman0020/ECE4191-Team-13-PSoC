...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_RPi) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_RPi_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_RPi_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_RPi_GetRxBufferSize();
                CyDelay(UART_RPi_BYTE2BYTE_TIME_OUT);
            }
            while(UART_RPi_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_RPi_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_RPi_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(UART_RPi_GetRxBufferSize() > 0u)
    {
        tempCount = UART_RPi_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_RPi_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_RPi_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_RPi_PACKET_EOP)
            {
                CyDelay(UART_RPi_WAIT_EOP_DELAY);
            }
        }
