...
uint32 Timer_2_ReadPeriod(void) 
{
   ...
   #else
       return (CY_GET_REG24(Timer_2_PERIOD_LSB_PTR));
   #endif /* (Timer_2_UsingFixedFunction) */
}
...
void Timer_2_WritePeriod(uint32 period) 
{
    ...
    #else
        CY_SET_REG24(Timer_2_PERIOD_LSB_PTR, period);
...
uint32 Timer_2_ReadCapture(void) 
{
   ...
   #else
       return (CY_GET_REG24(Timer_2_CAPTURE_LSB_PTR));
   #endif /* (Timer_2_UsingFixedFunction) */
}
...
void Timer_2_WriteCounter(uint32 counter) 
{
    ...
    #else
        CY_SET_REG24(Timer_2_COUNTER_LSB_PTR, counter);
