...
void LIS3MDL_Begin() {
    ...
    I2C_1_I2CMasterSendStart(LIS3MDL_I2CADD,I2C_1_WRITE_XFER_MODE,100);    // Initialize a transaction for writing
...
void LIS3MDL_ReadWhoAmIReg(uint8 * rdPtr) {
    ...
    Err1 = I2C_1_I2CMasterSendStart(LIS3MDL_I2CADD, I2C_1_WRITE_XFER_MODE, 1000);     // (M)ST | (M) SAD + W | (S) SAK
    Err2 = I2C_1_I2CMasterWriteByte(LIS3MDL_REG_WHO_AM_I,1000);                           // (M) SUB | (S) SAK
    Err3 = I2C_1_I2CMasterSendRestart(LIS3MDL_I2CADD, I2C_1_READ_XFER_MODE, 1000);  // (M) SR | (M) SAD + R | (S)SAK
    Err4 = I2C_1_I2CMasterReadByte(I2C_1_NAK_DATA, rdPtr, 1000);                   // (S) DATA | (M) NMAK
...
void LIS3MDL_ReadAxisRaw(uint8 * rdPtrL, uint8 * rdPtrH) {
    ...
    Err3 = I2C_1_I2CMasterSendRestart(LIS3MDL_I2CADD, I2C_1_READ_XFER_MODE, 1000);  // (M) SR | (M) SAD + R | (S)SAK
    Err4 = I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, rdPtrL, 1000);                   // (S) DATA | (M) NMAK
    Err5 = I2C_1_I2CMasterReadByte(I2C_1_NAK_DATA, rdPtrH, 1000);                // (S) DATA | (M) NMAK
...
void LIS3MDL_ReadCtrlReg(uint8 * CtrlReg1,uint8 * CtrlReg2, uint8 * CtrlReg3, uint8 * CtrlReg4) {
    ...
    Err3 = I2C_1_I2CMasterSendRestart(LIS3MDL_I2CADD, I2C_1_READ_XFER_MODE, 1000);  // (M) SR | (M) SAD + R | (S)SAK
    Err4 = I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, CtrlReg1, 1000);                 // (S) DATA | (M) MAK
    Err4 = I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, CtrlReg2, 1000);        
    Err4 = I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, CtrlReg3, 1000);                 
    Err5 = I2C_1_I2CMasterReadByte(I2C_1_NAK_DATA, CtrlReg4, 1000);                 // (S) DATA | (M) NMAK
    Err6 = I2C_1_I2CMasterSendStop(100); 
    char string[80]; sprintf(string, "Errors %ld %ld %ld %ld %ld %ld\t", Err1,Err2,Err3,Err4,Err5,Err6); UART_PutString(string); 
}
...
void LSM6DS3_Begin() {
    ...
    /*I2C_1_I2CMasterSendStart(LSM6DS3_I2CADD,I2C_1_WRITE_XFER_MODE,100);    // Initialize a transaction for writing
...
void LSM6DS3_ReadWhoAmIReg(uint8 * rdPtr) {
    /* (M)ST | (M)SAD+W| (S)SAK | (M)SUB | (S)SAK | (M)SR | (M)SAD+R | (S)SAK | (S)DATA | (M)NMAK | (M)SP |  */
    int32 Err1, Err2, Err3, Err4, Err5;
    Err1 = I2C_1_I2CMasterSendStart(LSM6DS3_I2CADD, I2C_1_WRITE_XFER_MODE, 1000);     // (M)ST | (M) SAD + W | (S) SAK
    Err2 = I2C_1_I2CMasterWriteByte(LSM6DS3_REG_WHO_AM_I,1000);                           // (M) SUB | (S) SAK
    Err3 = I2C_1_I2CMasterSendRestart(LSM6DS3_I2CADD, I2C_1_READ_XFER_MODE, 1000);  // (M) SR | (M) SAD + R | (S)SAK
    Err4 = I2C_1_I2CMasterReadByte(I2C_1_NAK_DATA, rdPtr, 1000);                   // (S) DATA | (M) NMAK
    Err5 = I2C_1_I2CMasterSendStop(100);                                                 // (M) SP
    char string[80]; sprintf(string, "\nErrors %ld %ld %ld %ld %ld ", Err1,Err2,Err3,Err4,Err5); UART_PutString(string); 
}
...
void LSM6DS3_ReadCtrlReg(uint8 CtrlReg[10]) {
    /* (M)ST | (M)SAD+W| (S)SAK | (M)SUB | (S)SAK | (M)SR | (M)SAD+R | (S)SAK | (S)DATA | (M)MAK | DATA| MAK | DATA | NMAK | (M)SP |  */
    I2C_1_I2CMasterSendStart(LSM6DS3_I2CADD, I2C_1_WRITE_XFER_MODE, 1000);   // (M)ST | (M) SAD + W | (S) SAK
    I2C_1_I2CMasterWriteByte(LSM6DS3_REG_CTRL1_XL,1000); //(M) SUB | (S) SAK         
    I2C_1_I2CMasterSendRestart(LSM6DS3_I2CADD, I2C_1_READ_XFER_MODE, 1000);  // (M) SR | (M) SAD + R | (S)SAK
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[0], 1000);                 // (S) DATA | (M) MAK
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[1], 1000);        
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[2], 1000);
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[3], 1000);                 
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[4], 1000);        
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[5], 1000);
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[6], 1000);                 
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[7], 1000);        
    I2C_1_I2CMasterReadByte(I2C_1_ACK_DATA, &CtrlReg[8], 1000);
    I2C_1_I2CMasterReadByte(I2C_1_NAK_DATA, &CtrlReg[9], 1000);                 // (S) DATA | (M) NMAK
    I2C_1_I2CMasterSendStop(100); 
    //char string[80]; sprintf(string, "Errors %ld %ld %ld %ld %ld %ld\t", Err1,Err2,Err3,Err4,Err5,Err6); UART_PutString(string); 
}